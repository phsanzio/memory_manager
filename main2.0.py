...
def nru(array_list, n_molduras, ciclo_clock):
    ...
    last_access = {}  # Dicionário para armazenar o último ciclo de acesso de cada página
    ...
    for clock in range(len(array_list)):
        ...
        # Atualizar o último ciclo de acesso da página
        last_access[pagina_atual] = clock
        ...
        # Redefinir os bits R a cada ciclo_clock ciclos, mas apenas para páginas que não foram acessadas
        if clock % ciclo_clock == 0:
            for pagina in ram:
                if clock - last_access[pagina] >= ciclo_clock:
                    bits_R[pagina] = 0  # Redefinir o bit R de páginas não acessadas
